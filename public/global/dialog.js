                ...
                if (this.index == 0) {
                    if (!!opts.persistent) {
                        try {
                            ...
                            if (data && data.top)
                                top = parseInt(/\d+/.exec(data.top)[0]);
                            if (data && data.left)
                                left = parseInt(/\d+/.exec(data.left)[0]);
                        } catch (e) {
                        }
                    }
...
`).join("") + "}\n" + Object.entries(opts.css).map((kv) => kv[0] == "window" ? kv[1].replace("{{ClassName}}", dlgWinName) : kv[1]).join("\n");
        ...
        return {
          ...
          .dlg-box .dlg-body input:focus { outline: var(--dlg-input-outline-focus); }`,
                    footer: `.dlg-box .dlg-foot{ padding: 5px 5px 0px 5px; text-align: right; }
          ...
          .dlg-box .dlg-foot button:hover {
            ...
            close: (id, ...args) => {
                if (Dialog.openDialogs.ids.length == 0)
                    return;
                const item = id != void 0 ? Dialog.openDialogs.ids.find((x) => x.id === id) : Dialog.openDialogs.ids[Dialog.openDialogs.ids.length - 1];
                if (item !== void 0)
                    item.instance.close(...args);
                else
                    throw new Error(`Cannot find Dialog with id=${id}`);
            },
            deepClone: (obj, override = void 0, filterKeys = () => true) => {
                let res = {};
                function doMerge(dst, src, path) {
                    for (const [key, val] of Object.entries(src)) {
                        if (!filterKeys(path, key))
                            continue;
            ...
            dragElement: (element, header, opts = {}) => {
                ...
                if (element == void 0 || typeof element != "object")
                    throw `Cannot find element: ${element}`;
                if (header == void 0 || typeof element != "object")
                ...
                const closeDragElement = () => {
                    ...
                    if (!!opts.persistent)
                        localStorage.setItem(opts.persistent, JSON.stringify({ top: element.offsetTop, left: element.offsetLeft }));
                };
            ...
            alert: (title, body, action, opts = {}) => new AlertBase(opts.element, "Alert", title, body, `<button class="default" onclick="Dialog.close(${opts.element})">Далі</button>`, typeof action == "object" ? action.action : action, typeof action == "object" ? action : opts),
            confirm: (title, body, action, opts = {}) => {
                if (typeof action == "object") {
                    opts = action;
                    action = opts.action;
                }
                const btns = opts.buttons || [{ title: "Далі" }, { title: "Відмінити" }];
                const okbtn = opts.btnOk || 0;
                const defbtn = opts.defbtn || 0;
                const foot = btns.map((b, idx) => {
                    const keys = Object.keys(b).filter((k) => k != "value");
                    return `<button${defbtn == idx ? ' class="default"' : ""} onclick="Dialog.close(${opts.element}, ${idx == okbtn})"` + keys.map((k) => ` ${k}=${b[k]}`).join("") + `>${b.value ? b.value : b.title}</button>
`;
                }).join("");
            ...
            prompt: (title, body, action, opts = {}) => {
                ...
                const buttons = opts.buttons || [{ title: "Далі" }, { title: "Відмінити" }];
                const defBtn = opts.defbtn || 0;
                const dlgId = opts.element;
                body += inputs.map((i, idx) => `<label for="${i.id}" text="${i.label}"/><input id="${i.id}"` + Object.keys(i).filter((k) => k != "label").map((k) => ` ${k}="${i[k]}"`).join("") + ">").join("");
                const foot = buttons.map((i, idx) => `<button ${idx == defBtn ? " class='default'" : ""}` + Object.keys(i).filter((k) => k != "value").map((k) => ` ${k}="${i[k]}"`).join("") + ` onclick="Dialog.close(${dlgId}, ${idx})">${i.value ? i.value : i.title}</button>
`).join("");
                const onclose = (btnIdx) => {
                    const args = inputs.map((i) => {
                        ...
                        const value = hasv ? input.value : void 0;
                        return { id: i.id, value };
                    });
                    const res = { ok: btnIdx == defBtn, button: btnIdx, values: args };
                    return typeof action === "function" ? action(res) : action !== void 0 && window[action] ? window[action](res) : res;
                };
